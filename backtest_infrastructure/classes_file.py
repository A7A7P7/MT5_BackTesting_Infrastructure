...
"""
Probable Inputs needed to test strategies:

For indicators in each timeframe:

    - The numbers used in each indicator in each timeframe

For structure:

    - Also the number from which the number of local_highs or local_lows are calculated


"""

"""FOR A STRATEGY YOU NEED TO CHANGE:
    
         - GivenTF_Strat_Signal Class : 
                                        - M1,M5,M15 based on what you use for your strategy
        OR:
                                        - add methods to each GivenTF_Strat_Signal to everytime timeframe usage;

        - Engine Class : 
                          - Change the 'signal_generation' in self.strategy.signal_generation to the new methods
                          - Based on the strategy also on the Engine Class the property indicators also needs to change

        - RiskModel Class : 
                             - If you want to change the RR and set different adjustments to just like SL & TP

        - MultiTimeframeEngine Class : 
                                        - Change properties where there is timeframes to the timeframes, one will use.
                                        - If not FX, create on MarketHoursOpen Class the Opening and Closing Hours of the market that you were operating.

        - ConfluenceEngine Class : 
                                    - Change properties where there is timeframes to the timeframes, one will use.
                        
        - ExecutionEngine Class :
                                    - Where it has timeframes such as 'm1', 'm5','m15' change to the timeframes that will be used and erase any unused
"""
...
class Portfolio:
    ...
    def update_portfolio_new_trade(self,position:Position):
        ...
        self.disposable_balance = self.disposable_balance - position.capital_at_risk
        self.unrealized_balance = self.unrealized_balance + position.value_added
    ...
    def update_portfolio(self,position:Position,last_candle:Candle):
        ...
        if position.result == None:
            ...
            if position.result == "TP":
                ...
                self.disposable_balance = self.disposable_balance + position.capital_at_risk + position.pnl
                self.unrealized_balance = self.unrealized_balance + position.value_added #HERE VALUE ADDED HAS ALREADY THE COMMISSION
                self.equity_curve = self.equity_curve + position.pnl
        ...
        self.lst_disposable_balance.append(self.disposable_balance)
...
class MultiTimeframeEngine:
    ...
    def __init__(self,engines:dict,buffers:dict,timeframes:dict,last_htf_candle:dict,next_expected_open:dict,latest_setups:dict,structures_when_trade_is_entered:dict):
        ...
        self.every_structure = {
            'm1' : [],
            'm5' : [],
            'm15' : []
        }
        self.every_candles_in_range_list = {
            'm1' : [],
            'm5' : [],
            'm15' : []
        }
    ...
    def on_m1_candle(self,candle:Candle):
        ...
        # always update M1
        self.latest_setups['m1'] = self.engines['m1'].on_candle(candle) #THIS SENDS AN INITIAL SIGNAL WHICH IS A SETUP EVENT OR NONE
        self.every_structure['m1'].append(deepcopy(self.engines['m1'].context.structure_snapshot))
        self.every_candles_in_range_list['m1'].append(deepcopy(self.engines['m1'].structure.candles_in_range))
        self.engines['m1'].structure.candles_m1_per_each_candle = 1
...
class ConfluenceEngine:
    ...
    def is_valid_confluence(self):
        ...
        m1 = self.latest_setup["m1"]
        m5 = self.latest_setup["m5"]
        m15 = self.latest_setup["m15"]
